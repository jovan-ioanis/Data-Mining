...
def kernel(X):
    ...
    # sigma = 0.11
    covariance = np.identity(nFeatures)
    mean = np.zeros(nFeatures)*sigma
    weightsRFF = np.random.multivariate_normal(mean, covariance, nRFF/2)
...
def SGD(label, featureMatrix):
    ...
    for t in range(nBatch):
        #localGradient = preGradient[miniBatch[:,t],:]*(weights)
